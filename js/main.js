...
input_search.addEventListener("input", () => search_gifs(input_search.value))
theme_button.addEventListener("click", () => handle_theme(true))

function search_gifs(query = "") {
    query = query.trim().toLocaleLowerCase().replaceAll(" ", "_")
    gif_container.innerHTML = ""

    GIF.forEach((gif) => {
        gif = gif.toLocaleLowerCase()
        if (gif.includes(query) || query == "") {
            let element = document.createElement("img")
            element.classList.add("gif")
            element.src = `storage/gif/${gif}`
...
function handle_theme(toggle = false) {
    let preferred = (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)')).matches; // true - dark, false - light
    let theme = ""
    ...
    if (toggle) {
        if (theme = localStorage.getItem("theme")) {
            ...
            theme = preferred ? "light" : "dark"    // opposite of preferred
        }
    }
    else {
        if (theme = localStorage.getItem("theme")) {
    ...
    theme_button.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="32px" width="32px" viewBox="0 -960 960 960" fill="#e3e3e3">${theme_icons[theme]}</svg>`
}
...
window.addEventListener("keydown", (e) => {
    ...
    if (
        e.key.charCodeAt() >= 48 && e.key.charCodeAt() <= 57 || // number
        e.key.charCodeAt() >= 65 && e.key.charCodeAt() <= 90 || // alphabet upper case
        e.key.charCodeAt() >= 97 && e.key.charCodeAt() <= 122   // alphabet lower case
    ) {
        input_search.focus()
    }
})
